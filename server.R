...
table<-function(x){
  ...
  avg<-((g1+g2)/2)
  df<-rbind(data3[x,-1],c(" ",g1,g2),c(avg,"",""))
  row.names(df)<-c("Exp. $","YoY Growth %","Avg. Growth %")
  return(df)
}
   #renderDataTable( )
# output$countryChosen <- reactiveText(function() {
   #as.character(input$country)
 #})    

#selectInput("dataset", "Data set", as.list(data3$Countries))
#output$countries <- reactiveText(function() {
#as.character(data3$Countries)
#})  
